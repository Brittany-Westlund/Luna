...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (lunaRb != null)
            lunaInitialGravity = lunaRb.gravityScale;
    ...
    private void HandleButterflyMovement(float speed)
    {
        ...
        // fallback to keyboard if axis is zero
    ...
    private void HandleButterflyFlip()
    {
        float hx = Input.GetAxis("Horizontal");
        if (Mathf.Abs(hx) < 0.01f)
            hx = Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.LeftArrow) ? -1f
    ...
    private void HandleFlightTimer()
    {
        ...
        {
            Dismount(jumpOff: false);  // time’s up → just fall off
        }
    }
