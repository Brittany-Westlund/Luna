...
public class SproutAndLightManager : MonoBehaviour
{
    ...
    public float growthIncrement = 0.1f; // Increased increment to hit max scale sooner
    ...
    void ShowSporeHintBasedOnScale()
    {
        ...
        else if (scale >= 0.10f && scale < 0.20f)
        {
            selectedOffset = hintOffsetStage2;
            selectedScale = hintScaleStage2;
        }
        else if (scale >= 0.20f && scale < maxScale) // FINAL spore stage only
        {
