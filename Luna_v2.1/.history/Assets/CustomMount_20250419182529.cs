...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    public Transform sporeHoldPoint;          // child of Luna
    public Transform flightSporeAttachPoint;  // child of butterfly
    ...
    void Start()
    {
        ...
        // ensure spore attaches to Luna’s hand initially
    ...
    void Update()
    {
        ...
        // R now: create + give in one
        if (Input.GetKeyDown(KeyCode.R))
        {
            // 1) Create the spore
            sporeSystem.CreateSpore();
            // 2) Immediately extend flight (if in warning window)
            if (canBeExtended)
                ExtendFlightWithSpore();
        }
    ...
    /// <summary>
    /// Called by TeaRosePollenPickup to apply a pollen speed boost.
    /// </summary>
