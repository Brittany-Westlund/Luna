...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    [Tooltip("How many to collect for a permanent boost")]
    public int   pollenThreshold     = 3;

    // exposed so LunaSporeSystem can read it
    ...
    void Update()
    {
        ...
        if (isFlying)
        {
            ...
            if (hasTempBoost)
            {  
                hasTempBoost = false; // consume it once
                teaRosePollenFX?.SetActive(false);
                Debug.Log("🚀 Temp pollen boost used!");
            }
            ...
            // R to extend with spore
            if (Input.GetKeyDown(KeyCode.R) && sporeSystem.HasSporeAttached && canBeExtended)
                ExtendFlightWithSpore();

            // Jump to dismount & leap
    ...
    private void Dismount()
    {
        ...
        // set justDismounted so spore‑system skips immediate detach
        justDismounted = true;
    ...
    {
        isInCooldown      = false;
        justDismounted    = false;           // clear the flag here
        butterflyRenderer.color = normalColor;
    }

    /// <summary>
    /// Called from TeaRosePollenPickup to apply pollen boosts.
    /// </summary>
