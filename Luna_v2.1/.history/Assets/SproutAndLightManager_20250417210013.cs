...
public class SproutAndLightManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // ✅ Initialize isPlanted only if this flower starts under a GardenSpot
        bool isChildOfGarden = transform.parent != null && transform.parent.GetComponent<GardenSpot>() != null;
        bool isInScene = gameObject.scene.IsValid();

        isPlanted = isChildOfGarden && isInScene;
        isHeld = false;

        Debug.Log($"{gameObject.name} initialized: isPlanted = {isPlanted}, isHeld = {isHeld}");
    }
   ...
   void Start()
    {
        ...
        // 🛠️ Smart init for isPlanted
        bool isInGarden = transform.parent == null || transform.parent.GetComponent<GardenSpot>() != null;
        bool isInScene = gameObject.scene.IsValid();

        if (isInGarden && isInScene)
        {
            isPlanted = true;
            isHeld = false;
        }
