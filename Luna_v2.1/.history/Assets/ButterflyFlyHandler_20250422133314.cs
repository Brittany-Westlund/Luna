...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    void Mount()
    {
       ...
       // Clear ghost visuals in flight hold points
        foreach (Transform child in anemonePollenFlightHoldPoint)
        {
            child.gameObject.SetActive(false);
        }

        foreach (Transform child in foxglovePollenFlightHoldPoint)
        {
            child.gameObject.SetActive(false);
        }

        // Move anemone pollen icon into flight hold point
        if (anemonePollenGroundHoldPoint.childCount > 0)
        {
            ...
            icon.gameObject.SetActive(true); // re-enable it after move
        }

        // Move foxglove pollen icon into flight hold point
   ...
   void Dismount(bool jumpOff)
    {
       ...
       // Move anemone pollen icon back to ground hold point
        ...
        // Move foxglove pollen icon back to ground hold point
