...
public class Aid : MonoBehaviour
{
    ...
    private IEnumerator SlideSporeForward(GameObject spore, Vector3 direction)
    {
        ...
        // Visualize the OverlapCircle detection area
        Debug.DrawRay(targetPosition, Vector3.up * 0.5f, Color.red, 1f);    // Up
        Debug.DrawRay(targetPosition, Vector3.down * 0.5f, Color.red, 1f);  // Down
        Debug.DrawRay(targetPosition, Vector3.left * 0.5f, Color.red, 1f);  // Left
        Debug.DrawRay(targetPosition, Vector3.right * 0.5f, Color.red, 1f); // Right
        ...
        Collider2D[] hitColliders = Physics2D.OverlapCircleAll(targetPosition, 1f); // Larger radius for debugging
        ...
        else
        {
            foreach (var collider in hitColliders)
            {
                ...
                if (collider.transform.root.CompareTag("Butterfly"))
                {
                    AidButterfly();
                    break;
                }
                else if (collider.transform.root.CompareTag("Flower"))
                {
                    AidFlower();
                    break;
                }
    ...
    private void AidFlower()
    {
        ...
        // Add specific behavior for flowers
        // Example: Make the flower bloom
    }
   ...
   private void AidButterfly()
    {
        ...
        // Use the detected collider to find the ButterflyFlyHandler script
        GameObject butterflyObject = GameObject.FindWithTag("Butterfly");
        if (butterflyObject != null)
        {
            ButterflyFlyHandler butterflyFlyHandler = butterflyObject.GetComponent<ButterflyFlyHandler>();
            if (butterflyFlyHandler != null)
            {
                Debug.Log("ButterflyFlyHandler found. Resetting cooldown.");
                butterflyFlyHandler.ResetCooldown();
            }
            else
            {
                Debug.LogError("ButterflyFlyHandler not found on the butterfly GameObject!");
            }
        }
        else
        {
            Debug.LogError("No GameObject tagged as Butterfly was found.");
        }
    }
