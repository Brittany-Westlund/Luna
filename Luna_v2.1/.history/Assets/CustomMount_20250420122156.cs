...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    void ExtendFlight()
    {
        // Push the timer back by the warning interval
        _flightTimer = Mathf.Max(0f, _flightTimer - warningTime);

        // Destroy the spore immediately
        _spore.DestroyAttachedSpore();

        // **Reset for the next warning cycle:**
        butterflyRenderer.color = normalColor;   // back to normal tint
        sparklePrefab?.SetActive(true);          // show sparkles again
        _warningTriggered = false;               // allow warning to fire again
