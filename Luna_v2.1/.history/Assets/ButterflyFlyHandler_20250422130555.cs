...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    void Mount()
    {
        ...
        Transform pollenAnchor = GameObject.Find("PollenIconAnchor").transform;

        // move anemone icon
        if (anemonePollenGroundHoldPoint.childCount > 0)
        {
            var icon = anemonePollenGroundHoldPoint.GetChild(0);
            icon.SetParent(pollenAnchor, true);
            icon.position = butterfly.position + new Vector3(0f, 0.5f, -0.1f); // adjust as needed
        ...
        if (foxglovePollenGroundHoldPoint.childCount > 0)
        {
            ...
            icon.SetParent(pollenAnchor, true);
            icon.position = butterfly.position + new Vector3(0f, 0.7f, -0.1f); // different offset if you want!
            icon.rotation = Quaternion.identity;
        }
   ...
   void Dismount(bool jumpOff)
    {
        ...
        // Return pollen icons from anchor to Luna
        if (pollenAnchor != null)
        {
            foreach (Transform icon in pollenAnchor)
            {
                if (icon.name.Contains("AnemonePollenLuna"))
                {
                    icon.SetParent(anemonePollenGroundHoldPoint, true);
                    icon.localPosition = Vector3.zero;
                    icon.localRotation = Quaternion.identity;
                }
                else if (icon.name.Contains("FoxglovePollenLuna"))
                {
