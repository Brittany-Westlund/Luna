...
public class PresentSpore : MonoBehaviour
{
    ...
    public int healthRestoreAmount = 0; // Amount of health regained when the spore is detached
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.S)) // Toggle spore on/off with 'S'
        {
            ...
            {
                CreateSpore();
                health -= healthLossAmount; // Decrease health when the spore is attached
            }
            else
            {
                DetachSpore();
                health += healthRestoreAmount; // Restore health when the spore is detached
            }
