...
public class SproutAndLightManager : MonoBehaviour
{
    ...
    /// <summary>Call when a spore hits this sprout.</summary>
    public void ResetOnGrowth()
    {
        ...
        transform.localScale = worldScaleTarget;        // snap world‑scale
        transform.position   = worldPosTarget;          // snap world‑position
        transform.rotation   = wRot;

        transform.SetParent(parent, true);              // reattach, preserving world transform
        // =====================================================
    ...
    /// <summary>
    /// After any FlowerSway rotation, snap hints back to upright in world space.
    /// </summary>
    void LateUpdate()
    {
...
// Working Script with rotating hints
/* using UnityEngine;
