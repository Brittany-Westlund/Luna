...
{
    [Header("References")]
    public GameObject luna;                   // your main Luna (with LunaSporeSystem)
    ...
    public Transform sporeHoldPoint;          // child of Luna for holding a spore
    public Transform flightSporeAttachPoint;  // child of butterfly for flight mode
    ...
    void Start()
    {
        ...
        // gather and remember every collider involved
        ...
        // point the spore system at Luna's hold point by default
    ...
    void Update()
    {
        ...
        // flying: movement, flip, timer
        var speed = baseSpeed + bonusSpeed;
        HandleMovement(speed);
        HandleFlip();
        HandleTimer();

        // R still goes through LunaSporeSystem (create/aid/detach)
        // Space to jump off
        if (Input.GetButtonDown("Jump"))
            Dismount(true);

        // X to pick/plant mid‑air
    ...
    void Mount()
    {
        ...
        // disable physics bumping by turning every collider into a trigger
        ...
        // sync facing from Luna's localScale.x
    ...
    void Dismount(bool jumpOff)
    {
        ...
        // spore attach back to Luna's hold point
        sporeSystem.attachPoint = sporeHoldPoint;

        // restore collider trigger states
        foreach (var c in allColliders)
            c.isTrigger = originalIsTrigger[c];

        // restore physics on Luna
        if (lunaRb != null)
        {
        ...
        // jump impulse if Space‑dismount
