...
public class SproutAndLightManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (flowerSway != null)
        {
            flowerSway.enabled = false; // Start with Flower Sway disabled
        }

        if (PointsText != null)
        {
            PointsText.enabled = false; // Hide points text initially
        }
    }
   ...
   void Update()
    {
        // Show the spore hint icon if player is in range, flower isn't grown yet, and has a spore
        bool canGrow = !isFullyGrown && IsPlayerInRange() && PlayerHasSpore();

        if (sporeHintIcon != null)
        {
            sporeHintIcon.SetActive(canGrow);
        }

        // Handle activating light after flower is fully grown
        if (Input.GetButtonDown("Player1_LightActivation") && isFullyGrown && IsPlayerInRange())
        {
            ActivateLight();
        }
    }
    ...
    private bool PlayerHasSpore()
    {
        LunaSporeSystem lunaSpore = PlayerTransform.GetComponent<LunaSporeSystem>();
        return lunaSpore != null && lunaSpore.HasSporeAttached;
    }
}
