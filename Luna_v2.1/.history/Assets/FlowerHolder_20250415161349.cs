...
public class FlowerHolder : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (canInteract && closestGarden != null && Input.GetKeyDown(KeyCode.X))
        {
            ...
            {
                // Remove other flower if present (but not the one she's holding)
                if (plantedFlower != null && plantedFlower != currentFlowerObject)
                {
    ...
    private GameObject GetPlantedFlower(Transform plantingPoint)
    {
        ...
        foreach (Transform child in plantingPoint)
        {
            if (child != null && child.gameObject != currentFlowerObject)
            {
