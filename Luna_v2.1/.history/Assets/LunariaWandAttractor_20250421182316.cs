...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    {
        // 1) gather everything under Q‑radius
        var hits = Physics2D.OverlapCircleAll(transform.position, lightRadius);
        if (hits.Length == 0) return;

        // 2) pick the closest valid target (flower, teapot, lantern)
        var candidate = hits
          .Where(c =>
             // fully‑grown, unlit flower?
             (c.TryGetComponent<SproutAndLightManager>(out var spr)
               && spr.IsFullyGrown
               && !spr.litFlowerRenderer.enabled)
             // OR teapot?
             || c.TryGetComponent<TeapotLightReceiver>(out _)
             // OR unlit lantern?
             || ((c.CompareTag("IndoorLantern") || c.CompareTag("OutdoorLantern"))
                 && c.transform.Find("LitLantern") is Transform lit && !lit.GetComponent<SpriteRenderer>().enabled)
          )
        ...
        if (candidate.TryGetComponent<SproutAndLightManager>(out var flower))
        {
            ...
            // … enable LitFlowerB, destroy hint …
            ConsumeMote();
            return;
        }
        ...
        // 3c) Lantern logic
        var lanternLit = candidate.transform.Find("LitLantern")?.GetComponent<SpriteRenderer>();
        if (lanternLit != null && !lanternLit.enabled)
        {
            lanternLit.enabled = true;
            ConsumeMote();
        }
    }
