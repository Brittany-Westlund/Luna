...
public class TeacupReceiver : MonoBehaviour
{
    ...
    public void ReceiveTeacup(GameObject teacup)
    {
        ...
        // Fire the DialogueSystemTrigger on THIS NPC
        var trigger = GetComponent<PixelCrushers.DialogueSystem.DialogueSystemTrigger>();
        if (trigger != null)
        {
            trigger.OnUse();
            Debug.Log("DialogueSystemTrigger OnUse called on NPC!");
        }
        else
        {
            Debug.LogWarning("No DialogueSystemTrigger found on NPC!");
        }
    }
