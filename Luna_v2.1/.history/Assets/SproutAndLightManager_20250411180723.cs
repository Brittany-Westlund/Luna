...
public class SproutAndLightManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 🔄 Auto-assign LightBar using tag
        if (LightBar == null)
        {
        ...
        // Flower sway
        if (flowerSway == null) flowerSway = GetComponent<FlowerSway>();
        ...
        {
            Debug.LogWarning($"[SproutAndLightManager] {gameObject.name} is missing references. Did you forget Setup()?");
        }
    }
        ...
        if (LightBar != null && ScoreManager.Instance != null)
        {
            if (LightBar.BarProgress >= lightActivationCost)
            {
                ...
                if (LitFlowerRenderer != null)
                {
                    ...
                    Debug.Log($"✅ Lit sprite activated for {gameObject.name}");
                }
