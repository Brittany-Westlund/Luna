...
public class LunariaWandAttractor : MonoBehaviour
{
   ...
   private void TryIlluminate()
    {
        // 1) gather everything in the little radius
        var hits = Physics2D.OverlapCircleAll(transform.position, lightRadius);
        if (hits.Length == 0) return;

        // 2) filter to only the things we care about, then pick the closest:
        var candidate = hits
            .Where(c =>
            {
                // sprouts must be fully grown and not yet lit
                ...
                // teapot
                if (c.TryGetComponent<TeapotLightReceiver>(out var _))
                    return true;

                // lanterns must have a disabled LitLantern child
                if ((c.CompareTag("IndoorLantern") || c.CompareTag("OutdoorLantern")) 
                    && c.transform.Find("LitLantern") is Transform L 
                    && L.GetComponent<SpriteRenderer>() is SpriteRenderer ls 
                    && !ls.enabled)
                    return true;
        ...
        if (candidate.TryGetComponent<SproutAndLightManager>(out var flower))
        {
            ...
            // turn on its LitFlowerB child
            var litB = candidate.transform.Find("LitFlowerB");
            if (litB != null)
            {
                var sr = litB.GetComponent<SpriteRenderer>();
                if (sr != null)
                    sr.enabled = true;
            }
        ...
        // 4) teapot?
        if (candidate.TryGetComponent<TeapotLightReceiver>(out var teapot))
        {
            if (ConsumeMote())
        ...
        // 5) lantern?
        {
            var L = candidate.transform.Find("LitLantern");
            if (L != null && L.GetComponent<SpriteRenderer>() is SpriteRenderer lsr && !lsr.enabled)
            {
                lsr.enabled = true;
                ConsumeMote();
            }
    ...
    {
        // 1) Entering Luna’s range → allow pickup
        ...
        // 2) Wand is held & empty & collides with a LightMote → absorb it
        if (_isHeld 
            && !_hasLight 
