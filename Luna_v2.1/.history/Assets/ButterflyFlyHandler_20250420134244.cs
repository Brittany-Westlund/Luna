...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    void Mount()
    {
        ...
        if (_holder.HasFlower)
        {
            ...
            f.transform.SetParent(flightFlowerHoldPoint, false);
    ...
    void Dismount(bool jumpOff)
{
    ...
    // if still holding a flower, reparent back to ground hold‑point
    if (_holder.HasFlower)
    {
        var f = _holder.GetHeldFlower();
        f.transform.SetParent(groundFlowerHoldPoint, false);
        f.transform.localPosition = Vector3.zero;
    }
