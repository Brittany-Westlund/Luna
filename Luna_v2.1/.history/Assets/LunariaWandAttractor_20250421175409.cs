   ...
   private void TryIlluminate()
    {
        ...
        // filter to only unlit, valid targets, then pick the closest
        var candidate = hits
            .Where(c =>
            {
                ...
                {
                    var lit = c.transform.Find("LitLantern");
                    if (lit != null)
                    {
                        var ls = lit.GetComponent<SpriteRenderer>();
        ...
        {
            var lit = candidate.transform.Find("LitLantern");
            if (lit != null)
            {
                var ls = lit.GetComponent<SpriteRenderer>();
                if (ls != null && !ls.enabled)
