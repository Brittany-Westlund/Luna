...
public class FlowerHolder : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (canInteract && closestGarden != null && Input.GetKeyDown(KeyCode.X))
        {
            ...
            // If not holding a flower, pick one up (if there's one in the garden)
            ...
            // If holding a flower, plant it
            else if (HasFlower())
            {
                // If a flower is already there, remove it
    ...
    private void UpdateClosestGarden()
    {
        ...
        foreach (GardenSpot garden in nearbyGardens)
        {
            bool shouldHighlight = (garden == closestGarden && HasFlower());
            garden.SetHighlight(shouldHighlight);
        }
    }
