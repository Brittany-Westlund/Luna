...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    {
        // gather everything in the little radius
        Collider2D[] hits = Physics2D.OverlapCircleAll(transform.position, lightRadius);
        if (hits.Length == 0) return;

        // filter to only valid, unlit targets, then pick the closest one
        Collider2D candidate = hits
            .Where(c =>
            {
                // 1) fully‑grown, unlit sprout?
                if (c.TryGetComponent<SproutAndLightManager>(out var s)
                    && s.IsFullyGrown
                    && !s.litFlowerRenderer.enabled)
                    return true;
                // 2) teapot (always valid target)
                if (c.TryGetComponent<TeapotLightReceiver>(out _))
                    return true;
                // 3) unlit lantern?
                if ((c.CompareTag("IndoorLantern") || c.CompareTag("OutdoorLantern"))
                    && c.transform.Find("LitLantern") is Transform lt
                    && lt.TryGetComponent<SpriteRenderer>(out var lsr)
                    && !lsr.enabled)
                    return true;
                return false;
            })
        ...
        // — if it’s a sprout, light & grow it
        if (candidate.TryGetComponent<SproutAndLightManager>(out var sprout))
        {
            ...
            if (candidate.transform.Find("LitFlowerB")
                    ?.TryGetComponent<SpriteRenderer>(out var fsr) == true)
                fsr.enabled = true;

            Destroy(candidate.transform.Find("LightMoteIcon(Clone)")?.gameObject);
            ConsumeMote();
            return;
        }

        // — else, if it’s a teapot, activate brew state
        if (candidate.TryGetComponent<TeapotLightReceiver>(out var teapotReceiver))
        {
            if (ConsumeMote())
                teapotReceiver.ActivateBrewReadyState();
            return;
        }

        // — else it must be a lantern
        if (candidate.transform.Find("LitLantern")
                ?.TryGetComponent<SpriteRenderer>(out var lanternSR) == true
            && !lanternSR.enabled)
        {
...
public class TeapotLightReceiver : MonoBehaviour
    ...
    {
        // Leaving Luna’s range → disable pickup
        if (!_isHeld && other.CompareTag("Player"))
        {
            _canPickup = false;
        }
