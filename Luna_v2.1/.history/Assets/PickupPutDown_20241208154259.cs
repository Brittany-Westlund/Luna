...
public class PickupPutDown : MonoBehaviour
{
    ...
    private void HandlePickupOrDrop()
    {
        ...
        {
            Collider2D nearbyObject = Physics2D.OverlapCircle(transform.position, detectionRadius);
            if (nearbyObject != null)
            {
                GameObject targetObject = nearbyObject.transform.root.gameObject;
                ...
                {
                    Debug.Log($"Nearby object '{targetObject.name}' is not interactable.");
                }
            }
            else
            {
