...
{
    [Header("References")]
    public GameObject luna;                   // your main Luna (with SporeSystem)
    public GameObject lunaInFlight;           // in‑flight sprite parented to butterfly
    ...
    public float jumpForce       = 5f;       // space‑bar impulse
    ...
    public Transform groundSporeAttachPoint; // assign your ground attach
    public Transform flightSporeAttachPoint; // assign a child of lunaInFlight
    ...
    void Start()
    {
        ...
        // ensure groundSporeAttachPoint matches the system
    ...
    private void Mount()
    {
        ...
        // switch spore attach
        sporeSystem.enabled      = false;
        sporeSystem.attachPoint  = flightSporeAttachPoint;

        // disable *all* Luna colliders
        ...
        // sync facing from Luna's current scale.x
        bool lunaFacingLeft = luna.transform.localScale.x < 0f;
        butterflyRenderer.flipX = lunaFacingLeft;
        var ls = lunaInFlight.transform.localScale;
        ls.x = lunaFacingLeft ? -Mathf.Abs(ls.x) :  Mathf.Abs(ls.x);
