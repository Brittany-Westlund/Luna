    ...
    private IEnumerator SlideSporeForward(GameObject spore, Vector3 direction)
{
    ...
    // Debug: Check all overlapping colliders
    ...
    else
    {
        ...
        {
            Debug.Log($"Found collider: {collider.name}, Tag: {collider.tag}, Layer: {LayerMask.LayerToName(collider.gameObject.layer)}");
        }
    }

    // Check for interaction with an NPC using tags
    Collider2D hitCollider = Physics2D.OverlapCircle(targetPosition, 0.5f); // Normal radius
    if (hitCollider != null)
    {
        Debug.Log($"Hit object: {hitCollider.name}, Tag: {hitCollider.tag}, Layer: {LayerMask.LayerToName(hitCollider.gameObject.layer)}");

        if (hitCollider.CompareTag("Flower"))
        {
            AidFlower();
        }
        else if (hitCollider.CompareTag("Butterfly"))
        {
            AidButterfly();
        }
        else
    ...
    {
        Debug.Log("No collider found at target position.");
    }
