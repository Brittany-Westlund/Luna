...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    void Mount()
    {
       ...
       // Set fatigue color explicitly
        if (_fatigue != null)
        {
            Color fatigueColor = _fatigue.GetFatigueColor();
            butterflyRenderer.color = fatigueColor;
            Debug.Log($"[Mount] Set butterflyRenderer.color to fatigue color: {fatigueColor}");
        }
   ...
   void Dismount(bool jumpOff)
    {
       ...
       // Set to cooldown color right now
        ...
        // Apply fatigue step, but do NOT change color yet
        if (_fatigue != null)
            _fatigue.ApplyFatigue(affectColor: false); // <-- overload this
