...
{
    [HideInInspector] public bool isHeld, isPlayerNearby;
    ...
    /// <summary>
    /// Growth steps (unchanged), except at the end we re-run hint logic if the player is still nearby.
    /// </summary>
    public void ResetOnGrowth()
    {
        ...
        if (_currentStage >= maxGrowthStages && !_isFullyGrown)
        {
            ...
            if (lightHintPrefab != null && _lightHintGO == null)
            {
                _lightHintGO = Instantiate(
                    ...
                    Quaternion.identity,
                    transform  // parent under this flower
                );
        ...
        // if Luna is still there, re‑attempt showing the right hint
        if (isPlayerNearby)
            TryShowHint();
    }

    /// <summary>
    /// Central gatekeeper for spawning hints.
    /// </summary>
    void TryShowHint()
    {
        // must be NOT held, playerNearby, AND (child‑of or near) a Garden
        if (isHeld || !isPlayerNearby || !IsInOrNearGarden())
        {
            HideSporeHint();
            HideLightHint();
            return;
    ...
    /// <summary>
    /// Checks if this flower is either a child under a GameObject tagged "Garden",
    /// OR within a small radius of any collider tagged "Garden".
    /// </summary>
    ...
    {
        // if picked up or dropped, re‑hide any hints and re‑check
        if (isHeld)
        {
            HideSporeHint();
            HideLightHint();
    ...
    /// <summary>
    /// Call externally (e.g. from TeapotReceiver) to wipe out both hints immediately.
    /// </summary>
