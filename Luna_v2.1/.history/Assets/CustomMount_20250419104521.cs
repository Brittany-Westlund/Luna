...
{
    [Header("References")]
    public GameObject luna;                   // Luna root (with LunaSporeSystem)
    public GameObject lunaInFlight;           // flight‐mode sprite parented under butterfly
    ...
    public float jumpForce       = 5f;       // upward impulse on space
    ...
    public Transform groundSporeAttachPoint; // assign in Inspector
    public Transform flightSporeAttachPoint; // assign to a child of lunaInFlight

    // **Needed by LunaSporeSystem for its justDismounted check**
    ...
    void Start()
    {
        ...
        // collect every collider we want to toggle
        allColliders.AddRange(butterfly.GetComponentsInChildren<Collider2D>());
        allColliders.AddRange(luna.GetComponentsInChildren<Collider2D>());
        allColliders.AddRange(lunaInFlight.GetComponentsInChildren<Collider2D>());

        // remember original isTrigger state
        ...
        // match the system’s ground attach
    ...
    void Update()
    {
        ...
        // F: mount / in‑flight: extend or dismount
        ...
        // R still creates/uses spore mid‑air, but F handles extend
        if (Input.GetKeyDown(KeyCode.R))
        {
            // let LunaSporeSystem run its Create/Aid/Detach logic
        }
    ...
    private void Mount()
    {
        ...
        // set all colliders to trigger to disable physics push
        foreach (var c in allColliders) c.isTrigger = true;
        ...
        // sync facing from Luna’s own scale
        bool left = luna.transform.localScale.x < 0f;
    ...
    private void Dismount(bool jumpOff)
    {
        ...
        // restore original collider trigger states
        foreach (var c in allColliders)
            c.isTrigger = originalIsTrigger[c];
