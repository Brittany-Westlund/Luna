...
public class FlowerInteraction : MonoBehaviour
{
    ...
    [Header("Teapot Hookup (auto‑detected)")]
    ...
    void Update()
    {
        // If holding a flower + in teapot range, let the teapot handle X
        if (heldFlower != null 
            && teapotReceiver != null 
            && teapotReceiver.PlayerIsNearby)
        {
            return;
        }
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        // Teapot detection
        else if (other.CompareTag("Teapot") && other.TryGetComponent<TeapotReceiver>(out var tr))
        {
            teapotReceiver = tr;
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        ...
        else if (other.CompareTag("Teapot") && other.GetComponent<TeapotReceiver>() == teapotReceiver)
        {
            teapotReceiver = null;
        }
    }
