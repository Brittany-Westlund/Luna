...
public class TeapotLightReceiver : MonoBehaviour
{
    ...
    public AudioSource brewAudioSource; // brewing sfx
    ...
    {
        // Play sound and delay reset ONLY ONCE
        if (brewAudioSource != null && brewAudioSource.clip != null)
        {
            Debug.Log("Brew sound!");
            brewAudioSource.Play();
            Invoke(nameof(ResetTeapot), brewAudioSource.clip.length);
        }
        else
        {
            Debug.LogWarning("brewAudioSource or its clip is null! Resetting immediately.");
            ResetTeapot(); // If there's no sound, reset right away
        }
        ...
        // Safety: if no sound, reset now (sound path will reset by Invoke).
        if (brewAudioSource == null || brewAudioSource.clip == null)
            ResetTeapot();
