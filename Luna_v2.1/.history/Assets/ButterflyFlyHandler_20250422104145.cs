...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    void Mount()
    {
        ...
        // move anemone pollen icon if it's active
        Transform anemoneIcon = anemonePollenGroundHoldPoint.childCount > 0 
            ? anemonePollenGroundHoldPoint.GetChild(0) : null;

        if (anemoneIcon != null && anemoneIcon.gameObject.activeSelf)
        {
            anemoneIcon.SetParent(anemonePollenFlightHoldPoint, true);
            anemoneIcon.localPosition = Vector3.zero;
            anemoneIcon.localRotation = Quaternion.identity;
        }

        // move foxglove pollen icon if it's active
        Transform foxgloveIcon = foxglovePollenGroundHoldPoint.childCount > 0 
            ? foxglovePollenGroundHoldPoint.GetChild(0) : null;

        if (foxgloveIcon != null && foxgloveIcon.gameObject.activeSelf)
        {
            foxgloveIcon.SetParent(foxglovePollenFlightHoldPoint, true);
            foxgloveIcon.localPosition = Vector3.zero;
            foxgloveIcon.localRotation = Quaternion.identity;
        }
    ...
    void Dismount(bool jumpOff)
{
    ...
    if (jumpOff && _rbLuna != null)
    {
       ...
       IEnumerator ReenableFollowNextFrame()
        ...
        // move anemone icon back to Luna
        ...
        // move foxglove icon back to Luna
