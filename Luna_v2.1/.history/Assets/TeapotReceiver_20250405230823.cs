...
public class TeapotReceiver : MonoBehaviour
{
    ...
    private void Start()
    {
        if (iconDisplayParent != null)
        {
            Transform icon = iconDisplayParent.Find("TeapotBrewIcon");
            if (icon != null)
            {
                brewIcon = icon.gameObject;
                brewIcon.SetActive(false);
            }
            else
            {
                Debug.LogWarning("⚠️ TeapotBrewIcon not found as a child of iconDisplayParent.");
            }
        }
        else
        {
            Debug.LogWarning("⚠️ iconDisplayParent is not assigned on the TeapotReceiver.");
        }
    }
