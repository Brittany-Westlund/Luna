...
public class SproutAndLightManager : MonoBehaviour
{
    ...
    /// <summary>
    /// After any FlowerSway (or other Update) runs, snap hints back upright.
    /// </summary>
    void LateUpdate()
    {
        if (_sporeHintGO != null)
            _sporeHintGO.transform.localRotation = Quaternion.identity;
        if (_lightHintGO != null)
            _lightHintGO.transform.localRotation = Quaternion.identity;
    }
}
