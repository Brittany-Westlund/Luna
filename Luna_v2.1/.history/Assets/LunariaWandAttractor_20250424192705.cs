...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    [Header("Audio")]
    public AudioClip lightMotePickupClip;
    public float pickupSoundVolume = 1.0f;   // Default: full volume
    public float pickupSoundPitch  = 1.0f;   // Default: normal pitch
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
            ...
            && Time.time >= _lastPickupTime + pickupCooldown)
        {
            ...
            // --- PLAY SOUND HERE ---
            if (lightMotePickupClip != null && audioSource != null)
            {
                audioSource.pitch = pickupSoundPitch;           // Set the pitch
                audioSource.PlayOneShot(lightMotePickupClip, pickupSoundVolume); // Play at specified volume
                audioSource.pitch = 1.0f; // (Optional: Reset pitch in case you use the AudioSource elsewhere)
            }
