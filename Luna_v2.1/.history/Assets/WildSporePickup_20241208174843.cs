...
public class WildSporePickup : MonoBehaviour
{
    ...
    private void HandlePickupOrDetach()
    {
        if (currentSpore == null) // Try to pick up a WildSpore
        {
            ...
            {
                Debug.Log($"Detected object: {nearbyCollider.name}, Tag: {nearbyCollider.tag}");

                if (nearbyCollider.CompareTag("Spore"))
                {
                    PickupSpore(nearbyCollider.gameObject);
                }
                else
                {
                    Debug.Log($"Object '{nearbyCollider.name}' is not a spore.");
                }
            }
            else
            {
                Debug.Log($"No spore nearby to pick up. Detection center: {transform.position}, Radius: {detectionRadius}");
            }
        }
