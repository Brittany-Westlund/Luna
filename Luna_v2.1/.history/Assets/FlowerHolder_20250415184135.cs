...
public class FlowerHolder : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // TEMP: Press F to debug all flower objects in scene
        if (Input.GetKeyDown(KeyCode.F))
        {
            Debug.Log("🌍 All FlowerPickup objects:");
            FlowerPickup[] allFlowers = FindObjectsOfType<FlowerPickup>();
            foreach (var flower in allFlowers)
            {
                Debug.Log($"• {flower.name} | Pos: {flower.transform.position} | Active: {flower.gameObject.activeInHierarchy} | Scale: {flower.transform.lossyScale}");
            }
        }
    }
