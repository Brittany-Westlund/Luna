...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // V: toggle wand pick‑up/drop (no longer gated by canPickup)
    ...
    private void TryIlluminate()
    {
        ...
        foreach (var col in hits)
        {
            ...
            if (spr != null)
            {
                ...
                // enable the LitFlowerB child if present
                var litFlowerB = col.transform.Find("LitFlowerB");
                if (litFlowerB != null && litFlowerB.TryGetComponent<SpriteRenderer>(out var fsr))
                    fsr.enabled = true;

                if (ConsumeMote()) return;
            }

            // 2) any lantern (IndoorLantern or OutdoorLantern)
            bool isLantern = col.CompareTag("IndoorLantern") || col.CompareTag("OutdoorLantern");
            if (isLantern)
            {
                var lit = col.transform.Find("LitLantern");
                if (lit != null && lit.TryGetComponent<SpriteRenderer>(out var lsr) && !lsr.enabled)
                {
                    lsr.enabled = true;
                    if (ConsumeMote()) return;
                }
    ...
    private void ResetWandVisuals()
    {
        ...
        /// <summary>
    /// Allows other scripts (like TeapotLightReceiver) to query if the wand currently holds light.
    /// </summary>
    ...
    /// <summary>
    /// Allows other scripts to hand off the light mote.
    /// Returns true if a mote was consumed.
    /// </summary>
    public bool GiveLightToObject()
    {
        if (!hasLight) return false;
        // consume the mote and reset visuals
        hasLight = false;
        ResetWandVisuals();
