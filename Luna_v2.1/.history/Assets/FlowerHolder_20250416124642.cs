    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.X))
        {
            ...
            if (HasFlower)
            {
                ...
                {
                    closest.PickUp()?.SetActive(false);
                }
            ...
            Invoke(nameof(ClearBusy), 0.1f); // Let everything settle
        }
    }
...
private void ClearBusy() => isBusy = false;
    ...
    private void PickUpFlower(GameObject flower)
    {
        ...
        // Recalculate only if the flower was not already held
        if (flower.transform.parent == null || flower.transform.parent.CompareTag("Garden"))
        {
