...
public class TeapotReceiver : MonoBehaviour
{
    ...
    public class FlowerIconEntry
    {
        ...
        public GameObject iconInstance; // Hierarchy-based assignment instead of prefab
    }
    ...
    private void Start()
    {
        ...
        foreach (var entry in flowerIconEntries)
        {
            ...
            {
                flowerIconMap.Add(entry.flowerType, entry.iconInstance);
                entry.iconInstance.SetActive(false); // Hide icon until it's needed
            }
        }
    ...
    {
        if (flowerIconMap.TryGetValue(flowerType, out GameObject icon))
        {
            icon.transform.SetParent(iconDisplayParent);
            int index = iconDisplayParent.childCount;
            Vector3 offset = new Vector3(0.5f * (index - 1), 0, 0);
        ...
        else if (iconPrefab != null)
        {
            ...
            Instantiate(iconPrefab, iconDisplayParent.position + offset, Quaternion.identity, iconDisplayParent);
        }
    ...
    public void BrewTea()
    {
        if (teaIngredients.Count > 0)
        {
            ...
            {
                child.gameObject.SetActive(false); // Hide instead of destroy
            }
