...
public class PresentSpore : MonoBehaviour
{
    ...
    public float healthGainOnAbsorb = 3f; // Health gain for absorbing a WildSpore
    public float absorbRadius = 1.0f; // Radius for detecting nearby WildSpores
    ...
    private WildSporePickup wildSporePickup; // Reference to WildSporePickup component
    ...
    void Start()
    {
        ...
        wildSporePickup = GetComponent<WildSporePickup>();
        if (wildSporePickup == null)
        {
            Debug.LogError("WildSporePickup component not found on Luna!");
        }
    ...
    void Update()
    {
        // Handle spore creation or detachment
        if (Input.GetKeyDown(KeyCode.S))
        {
            if (activeSpore == null)
            {
                if (wildSporePickup != null && wildSporePickup.IsHoldingSpore())
                {
                    Debug.Log("Cannot create a spore while holding a WildSpore.");
                    return;
                }
                CreateSpore();
            }
        ...
        // Handle absorbing nearby WildSpores
    ...
    void AbsorbNearbySpore()
    {
        Collider2D nearbySpore = Physics2D.OverlapCircle(transform.position, absorbRadius, LayerMask.GetMask("Spore"));
        if (nearbySpore != null)
        {
            Debug.Log("Absorbing WildSpore.");
            Destroy(nearbySpore.gameObject);

            _healthComponent.CurrentHealth = Mathf.Min(
                _healthComponent.CurrentHealth + healthGainOnAbsorb,
                _healthComponent.MaximumHealth
            );
        ...
        {
            Debug.Log("No nearby WildSpore to absorb.");
        }
    }
