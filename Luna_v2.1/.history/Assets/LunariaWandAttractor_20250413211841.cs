...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Make sure the wand starts dropped
        isHeld = false;
        ...
        // Just in case it was set as a child of something in the scene
   ...
   private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        else if (hasLight && other.CompareTag("Teapot"))
        {
            ...
            if (receiver != null)
            {
                receiver.ReceiveLight();     // ✅ Tell the teapot it's charged
                GiveLightToObject();         // ✅ Remove the wand's light and reset visuals
                Debug.Log("✨ Gave light to teapot: " + other.name);
            }
            else
            {
                Debug.LogWarning("⚠️ Teapot found but missing TeapotReceiver script.");
            }
        }
