...
public class PickupPutDown : MonoBehaviour
{
    ...
    public float dropOffset = 0.5f; // Distance in front of Luna when dropping objects
    ...
    private void Drop()
    {
        if (currentHeldObject != null)
        {
            ...
            Vector3 dropPosition = new Vector3(
                _player.transform.position.x + dropOffset * _player.transform.localScale.x, // Offset based on facing direction
                _player.transform.position.y,
                currentHeldObject.transform.position.z
            );

            // Check if the drop position is clear
            Collider2D overlap = Physics2D.OverlapCircle(dropPosition, 0.1f);
            if (overlap != null)
            {
                Debug.LogWarning("Drop position is obstructed. Adjusting drop offset.");
                dropPosition.x += 0.2f * _player.transform.localScale.x; // Adjust drop position
            }
