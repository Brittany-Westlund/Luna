...
public class ButterflyPatrol : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // If Luna is on the butterfly, check for movement and update her position
        if (luna != null)
        {
            // Check if Luna has moved
            if (IsLunaIdle())
            {
                luna.position = transform.position; // Keep Luna with the butterfly when idle
            }
            else
            {
                JumpOff(); // Detach Luna when she moves
            }
