...
public class SproutAndLightManager : MonoBehaviour
{
    ...
    // ── NEW: only show hints while touching a Garden ──
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        // only show if in a garden AND not currently held
        if (!isInGarden || isHeld) return;
    ...
    public void ResetOnGrowth()
    {
        ...
        transform.localScale = Vector3.one * (_initialWorldScale.x + growthIncrement * _currentStage);
        transform.position   = new Vector3(
            _initialWorldPos.x,
            Mathf.Min(_initialWorldPos.y + yPositionIncrement * _currentStage, maxHeight),
            _initialWorldPos.z
        );
        ...
        // rethink hints
        HideSporeHint();
        if (isPlayerNearby && _currentStage < maxGrowthStages && isInGarden && !isHeld)
            ShowSporeHint();
        ...
        if (_currentStage >= maxGrowthStages)
        {
            ...
            if (lightHintPrefab != null && _lightHintGO == null)
            {
                ...
                HideLightHint();
                if (isPlayerNearby && isInGarden && !isHeld)
                    ShowLightHint();
            }
        }
    }
    ...
    void ShowSporeHint()
    {
        ...
        _sporeHintOffset = (hintOffsets.Length > _currentStage)
            ? hintOffsets[_currentStage]
            : Vector3.up;
        float sc = (hintScales.Length > _currentStage)
    ...
    public void GiveLight()
    {
        ...
        // destroy light‐hint so it won’t reappear
    ...
    {
        // if Luna picks it up, immediately hide hints
        if (isHeld)
        {
            HideSporeHint();
            HideLightHint();
    ...
    /// <summary>
    /// External helper to clear both hints at once.
    /// </summary>
