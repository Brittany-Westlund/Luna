...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    public GameObject lunaInFlight;                 // Flying Luna sprite (child of butterfly)
    public Transform butterfly;                     // Butterfly mount transform
    ...
    public Transform flowerHoldPoint;
    ...
    void Start()
    {
        ...
        _flightOriginalParent = lunaInFlight.transform.parent;

        // Keep both GameObjects active; toggle only renderers and colliders
        lunaInFlight.SetActive(true);
        lunaInFlight.GetComponentInChildren<SpriteRenderer>().enabled = false;
        luna.GetComponentInChildren<SpriteRenderer>().enabled = true;
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.F) && !_inCooldown)
        {
            if (_isFlying)
            {
                if (_canExtend && _spore.HasSporeAttached)
                    ExtendFlight();
                else
                    Dismount(false);
            }
    ...
    void Mount()
    {
        ...
        // Show flight sprite, hide ground sprite
        var groundSR = luna.GetComponentInChildren<SpriteRenderer>();
        var flightSR = lunaInFlight.GetComponentInChildren<SpriteRenderer>();
        groundSR.enabled = false;
        flightSR.enabled = true;

        // Parent under butterfly and reset local position
        lunaInFlight.transform.SetParent(butterfly, true);
        lunaInFlight.transform.localPosition = Vector3.zero;
    ...
    void Dismount(bool jumpOff)
    {
        var groundSR = luna.GetComponentInChildren<SpriteRenderer>();
        var flightSR = lunaInFlight.GetComponentInChildren<SpriteRenderer>();
        flightSR.enabled = false;
        groundSR.enabled = true;
    ...
    Vector2 HandleMovement()
    {
        ...
        if (dir != Vector2.zero)
        {
            ...
            butterfly.Translate(dir * speed * Time.deltaTime, Space.World);
            if (_hasTempBoost)
            {
                _hasTempBoost = false;
