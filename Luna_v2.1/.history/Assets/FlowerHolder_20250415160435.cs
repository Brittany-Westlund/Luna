        ...
        if (canInteract && closestGarden != null && Input.GetKeyDown(KeyCode.X))
        {
            ...
            if (!HasFlower())
            {
                if (plantingPoint.childCount > 0)
                {
                    ...
                    // ✅ Only pick up the flower if it's not already held
                    if (flower != currentFlowerObject)
                    {
            ...
            else
            {
                if (plantingPoint.childCount > 0)
                {
                    ...
                    // ✅ Don’t interfere if the same flower is being replanted
                    if (existing != currentFlowerObject)
                    {
