...
public class FlowerHolder : MonoBehaviour
{
    ...
    private void Start()
    {
        if (holdPoint == null)
        {
            ...
            if (holdPoint == null)
            {
                Debug.LogWarning("⚠️ No HoldPoint assigned or found.");
            }
        }
    }
    ...
    private void Update()
    {
        ...
        if (canInteract && closestGarden != null && Input.GetKeyDown(KeyCode.X))
        {
            ...
            // If not holding a flower, pick up the one from this patch (if any)
            ...
            // If holding a flower, plant it here (replacing whatever was there)
            else if (HasFlower())
            {
    ...
    private void UpdateClosestGardenHighlight()
    {
        ...
        if (!HasFlower() || nearbyGardens.Count == 0)
        {
            closestGarden = null;
            return;
        }
        ...
        foreach (var g in nearbyGardens)
        {
            ...
            Transform plantingPoint = g.transform.Find("PlantingPoint");
