...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    private void TryIlluminate()
    {
        ...
        foreach (var col in hits)
        {
            ...
            // --- TEAPOT ---
            // if you have a TeapotLightReceiver on this collider…
            if (col.TryGetComponent<TeapotLightReceiver>(out var teapot))
            {
                // consume the mote first
                if (GiveLightToObject())
                {
                    teapot.ReceiveLight();   // your method that lights the teapot
                    Debug.Log("🫖 Teapot lit up!");
                    return;
                }
            }
    ...
    private void PickUpWand()
    {
        ...
        // We'll let Update() reparent us on the very next frame
        var rb = GetComponent<Rigidbody2D>(); if (rb!=null) rb.isKinematic=true;
