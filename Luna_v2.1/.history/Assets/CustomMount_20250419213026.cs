...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    [Header("Physical Colliders (drag here)")]
    public Collider2D butterflyPhysicalCollider;
    public Collider2D lunaPhysicalCollider;
    ...
    void Update()
    {
        ...
        // lock the in‑flight sprite to butterfly to prevent bobbing
        lunaInFlight.transform.position = butterfly.position;
    ...
    void Mount()
    {
        ...
        // disable both physical colliders to stop bobbing
        if (butterflyPhysicalCollider != null) butterflyPhysicalCollider.enabled = false;
        if (lunaPhysicalCollider      != null) lunaPhysicalCollider.enabled      = false;
        // disable any other non-trigger Luna colliders
        foreach (var c in _colsLuna) if (!c.isTrigger) c.enabled = false;
    ...
    void Dismount(bool jumpOff)
    {
        ...
        // re-enable both physical colliders
        if (butterflyPhysicalCollider != null) butterflyPhysicalCollider.enabled = true;
        if (lunaPhysicalCollider      != null) lunaPhysicalCollider.enabled      = true;
        // re-enable other Luna colliders
        foreach (var c in _colsLuna) if (!c.isTrigger) c.enabled = true;
