...
public class Aid : MonoBehaviour
{
    ...
    public float detectionRadius = 0.5f; // Radius for detecting aidable NPCs
    ...
    private IEnumerator SlideSporeForward(GameObject spore, Vector3 direction)
    {
        ...
        // Check for interaction with an NPC using a collider overlap
        Collider2D hitCollider = Physics2D.OverlapCircle(targetPosition, detectionRadius);
        if (hitCollider != null)
        {
            ...
            {
                Debug.Log("Butterfly detected. Triggering cooldown reset.");
                AidButterfly(hitCollider.gameObject);
            }
            else if (hitCollider.CompareTag("Flower"))
            {
                Debug.Log("Flower detected. Triggering AidFlower.");
                AidFlower();
            }
            else
            {
        ...
        {
            Debug.Log("No collider detected at target position.");
        }
    ...
    {
        Debug.Log("Aiding the butterfly!");
        ...
        if (butterflyFlyHandler != null)
        {
            ...
            butterflyFlyHandler.ResetCooldownExplicitly(); // Explicitly reset the cooldown
        }
        else
        {
