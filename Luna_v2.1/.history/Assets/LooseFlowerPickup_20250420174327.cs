...
public class LooseFlowerPickup : MonoBehaviour
{
    ...
    private void Pickup(GameObject go)
    {
        ...
        // ————————————————————————————————————————
        // Update SproutAndLightManager's saved initial position
        var sproutMgr = held.GetComponent<SproutAndLightManager>();
        if (sproutMgr != null)
        {
            // assume SproutAndLightManager has a private Vector2 field named "initialPosition"
            var type = typeof(SproutAndLightManager);
            var field = type.GetField("initialPosition",
                System.Reflection.BindingFlags.NonPublic |
                System.Reflection.BindingFlags.Instance);

            if (field != null)
            {
                // write the current world position so future ResetOnGrowth()
                // will use this spot instead of the original scene‐start spot
                field.SetValue(sproutMgr, (Vector2)held.transform.position);
            }
        }
        // ————————————————————————————————————————
    }
