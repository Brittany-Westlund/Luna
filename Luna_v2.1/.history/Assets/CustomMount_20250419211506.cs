...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    [Header("Luna Flight Slot")]
    [Tooltip("An empty child of 'butterfly'.  lunaInFlight will be parented here so it never bobs.")]
    public Transform lunaFlightSlot;
    ...
    Transform _lunaInFlightOriginalParent;
    ...
    void Start()
    {
        ...
        // save original parent so we can restore on dismount
        _lunaInFlightOriginalParent = lunaInFlight.transform.parent;

        // default spore attach on ground
        ...
        // initial facing from your sprite
    ...
    void Update()
    {
        ...
        {
            HandleMovement();
            HandleFacing();   // flips all three sprites now
            HandleTimer();
    ...
    void Mount()
    {
        ...
        // parent & snap to your flight slot so it never bobs
        lunaInFlight.transform.SetParent(lunaFlightSlot, false);
        lunaInFlight.SetActive(true);

        _spore.attachPoint     = flightSporeAttachPoint;

        // disable butterfly collider + ignore Luna collisions
    ...
    void Dismount(bool jumpOff)
    {
        ...
        // re-enable physical collider + collisions
        if (butterflyPhysicalCollider != null)
        ...
        // unparent flight so it doesn’t stick
        lunaInFlight.transform.SetParent(_lunaInFlightOriginalParent, true);

        // reposition real Luna
