...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        else if (hasLight && other.CompareTag("Lantern"))
        {
            SpriteRenderer litSprite = other.transform.Find("LitLantern")?.GetComponent<SpriteRenderer>();
            if (litSprite != null)
            {
                litSprite.enabled = true;
                GiveLightToObject();
            }
        }
        else if (hasLight && other.transform.childCount > 0)
        {
            Transform flower = other.transform;
            Transform lightHint = flower.Find("LightMoteIcon(Clone)");
            Transform litVersion = flower.Find("LitFlowerB");

            if (lightHint != null && litVersion != null)
            {
                // Hide unlit sprite
                ...
                GiveLightToObject();
                Debug.Log($"🌸 Illuminated flower: {other.name}");
            }
        }
    }
