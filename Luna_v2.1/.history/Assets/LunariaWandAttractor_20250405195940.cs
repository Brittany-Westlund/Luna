...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    public void DropWand()
    {
        ...
        if (luna != null)
        {
            ...
            transform.rotation = Quaternion.Euler(0f, 0f, 45f); // Reset to upright rotation
        }
    }
    ...
    public void TryPickupWand()
    {
        ...
        if (luna != null && Vector3.Distance(transform.position, luna.transform.position) < 2f)
        {
            ...
            transform.localRotation = Quaternion.Euler(0f, 0f, 45f); // Always pick up with upward 45 degree angle
