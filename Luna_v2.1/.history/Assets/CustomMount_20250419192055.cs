...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    // track last known facing from input
    ...
    void Update()
    {
        ...
        // if flying, handle flight controls
        if (isFlying)
        {
    ...
    void Mount()
    {
        ...
        // spore to flight point
        _spore.attachPoint = flightSporeAttachPoint;

        // ignore collisions butterfly<->Luna during flight
        ...
        // reset facing from last input‑based value
    ...
    void HandleFlip()
    {
        ...
        if (Mathf.Abs(hx) < 0.01f)
        {
            ...
            else if (Input.GetKey(KeyCode.D)) hx =  1f;
        }
        if (hx != 0f)
        {
            bool left = hx < 0f;
