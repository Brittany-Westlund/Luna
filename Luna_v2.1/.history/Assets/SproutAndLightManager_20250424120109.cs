...
public class SproutAndLightManager : MonoBehaviour
{
    ...
    public void ResetOnGrowth()
    {
        ...
        // after growth, re‑attempt hint if player still there
        HideSporeHint();
        HideLightHint();
        if (isPlayerNearby)
            TryShowHint();

        // spawn the light hint GO once fully grown
    ...
    public void GiveLight()
    {
        if (!IsInOrNearGarden())
        {
            Debug.Log("❗ Cannot illuminate this flower outside of a garden.");
            return;
        }
    ...
    public void BrewFlower()
    {
        if (hasBeenLit)
        {
            ...
            // Optionally: turn off lit visuals
            if (litFlowerRenderer != null)
                litFlowerRenderer.enabled = false;
        }
    }
