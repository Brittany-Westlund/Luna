...
public class ChangeColorOnBloomForLitFlowerB : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.CompareTag("Player"))
        {
            ...
            foreach (GameObject litFlowerB in litFlowerBObjects)
            {
                ...
                {
                    Debug.Log("LitFlowerB detected and enabled. Lightening background...");
                    AdjustBackgroundColor();
                    break; // Lighten only once per collision
                }
                else
                {
                    Debug.LogWarning("LitFlowerB renderer not enabled or missing.");
                }
            }
        }
        else
        {
            Debug.Log("Collision detected with object other than Player.");
        }
    }
    ...
    void AdjustBackgroundColor()
    {
        if (spriteRenderer != null)
        {
            ...
            // Log the new color value for debugging
