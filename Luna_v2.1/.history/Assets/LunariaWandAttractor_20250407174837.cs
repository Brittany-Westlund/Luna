...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        else if (hasLight)
        {
            ...
            {
                LanternTrigger lantern = other.GetComponent<LanternTrigger>();
                if (lantern != null && lantern.Illuminate())
                {
                    GiveLightToObject();
                }
            }

            // Flower logic by specific name and LightHintIcon
            string[] validFlowerNames = { "TeaRose", "Buttercup", "Goldenrod", "Anemone", "Iris", "Moonflower", "Foxglove" };

            foreach (string flowerName in validFlowerNames)
            {
                if (other.name == flowerName)
                {
                    Transform hintIcon = other.transform.Find("LightHintIcon");
                    if (hintIcon != null && hintIcon.gameObject.activeSelf)
                    {
                        Transform litVersion = other.transform.parent.Find("LitFlowerB");
                        if (litVersion != null)
                        {
                            litVersion.gameObject.SetActive(true);
                            other.gameObject.SetActive(false);
                            GiveLightToObject();
                            Debug.Log("🌸 Flower illuminated: " + flowerName);
                        }
                        else
                        {
                            Debug.LogWarning("🌑 LitFlowerB not found for: " + flowerName);
                        }
                    }
                }
            }
