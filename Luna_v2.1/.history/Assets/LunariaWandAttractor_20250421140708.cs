...
public class LunariaWandAttractor : MonoBehaviour
{
    ...
    private void TryIlluminate()
    {
        ...
        // filter only fully‑grown sprouts or lanterns, then pick the closest one
       var candidate = hits
        .Where(c =>
            // only include sprouts that are fully grown AND not yet lit
            (c.TryGetComponent<SproutAndLightManager>(out var spr)
                && spr.IsFullyGrown
                && !spr.litFlowerRenderer.enabled)
            || c.CompareTag("IndoorLantern")
            || c.CompareTag("OutdoorLantern")
        )
        ...
        // otherwise it's a lantern—light that
        var lit = candidate.transform.Find("LitLantern");
        if (lit?.TryGetComponent<SpriteRenderer>(out var lsr) == true && !lsr.enabled)
        {
            lsr.enabled = true;
            ConsumeMote();
        }
