...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    [Tooltip("Playback speed of the flying animation")]
    public float animationSpeedFlying = 2f;
    [Tooltip("Playback speed of the normal animation")]
    ...
    [Tooltip("Butterfly tint during normal flight")]
    public Color normalColor   = Color.white;
    [Tooltip("Butterfly tint when warning time is near end")]
    public Color warningColor  = Color.yellow;
    [Tooltip("Butterfly tint during cooldown/after dismount")]
    ...
    // cache original scales so we only ever flip X
    ...
    void Update()
    {
        // Mount / Extend / Dismount on F
        if (Input.GetKeyDown(KeyCode.F) && !_inCooldown)
        {
            if (_isFlying)
            {
                if (_spore.HasSporeAttached && _canExtend) ExtendFlight();
                else                                     Dismount(false);
            }
            else Mount();
            return;
        ...
        if (_isFlying)
        {
            ...
            // R: spawn & possibly consume + extend
    ...
    {
        // flip based on actual horizontal movement
        ...
        lunaInFlight.transform.localScale = new Vector3(
            _lunaOrigScale.x * sign,
            _lunaOrigScale.y,
            _lunaOrigScale.z
        );
    }
    ...
    {
        // instantly place Luna so she won’t “bob” up/down
        ...
        // reset facing using our own flag, not FollowAndFlip
        float groundSign = _isFacingRight ? +1f : -1f;
        butterfly.localScale = new Vector3(
            _butterflyOrigScale.x * groundSign,
            _butterflyOrigScale.y,
            _butterflyOrigScale.z
        );
    ...
    void HandleAirInteract()
    {
        ...
        {
            // note: HasFlower is a bool property, not a method
