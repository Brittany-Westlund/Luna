...
public class BackgroundLightAdjuster : MonoBehaviour
{
    ...
    public float colorIncreaseStep = 0.5f; // The percentage of color change towards white per trigger.
    ...
    void AdjustBackgroundColor()
    {
        ...
        foreach (GameObject obj in backgroundObjects)
        {
            if (obj.layer == LayerMask.NameToLayer("Background"))
            {
                ...
                if (renderer != null)
                {
                    // Log the object and its current color for debugging
                    Debug.Log("Adjusting color of: " + obj.name);
