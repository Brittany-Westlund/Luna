...
public class TeapotReceiver : MonoBehaviour
{
    ...
    void Update()
    {
        // Only handle X here; FlowerInteraction early‑outs when in range
    ...
    public void AddFlowerToTeapot(FlowerHolder holder)
    {
        ...
        if (ingredientIcons.TryGetValue(type, out var icon)) icon.SetActive(true);
    ...
    public void RetrieveLastFlower(FlowerHolder holder)
    {
        ...
        if (ingredientIcons.TryGetValue(type, out var icon)) icon.SetActive(false);
    ...
    public GameObject BrewTea()
    {
        ...
        brewIcon?.SetActive(false);
        foreach (var icon in ingredientIcons.Values) icon.SetActive(false);
