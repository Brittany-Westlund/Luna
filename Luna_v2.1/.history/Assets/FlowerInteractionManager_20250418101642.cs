...
public class FlowerInteraction : MonoBehaviour
{
    ...
    private void TryPlantFlower()
    {
        ...
        // Retrieve any flower already in this garden
        GameObject swapped = garden.GetPlantedFlower();
        if (swapped == null)
        {
            // fallback: any child under garden that isn't what we're holding
            foreach (Transform child in garden.transform)
            {
                if (child.gameObject != heldFlower && child.TryGetComponent<SproutAndLightManager>(out _))
                {
        ...
        // Swap back the previous flower (if any)
        if (swapped != null)
        {
    ...
    private GameObject FindClosestFlower()
    {
        ...
        foreach (var flower in nearbyFlowers)
        {
            ...
            // ensure it's a true sprout, not a garden object
            if (!flower.TryGetComponent<SproutAndLightManager>(out _))
                continue;
    ...
    {
        // Only track real Sprout objects
