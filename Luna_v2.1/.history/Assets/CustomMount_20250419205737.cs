...
public class ButterflyFlyHandler : MonoBehaviour
{
    ...
    /// <summary>
    /// Called by TeaRosePollenPickup when picking up pollen.
    /// Applies a temporary speed boost and accumulates for a threshold.
    /// </summary>
    public void ApplyTeaRosePollen(float speedBoost, int threshold)
    {
        teaRosePollenCount++;
        teaRosePollenFX?.SetActive(true);
        _rb.velocity = _rb.velocity + _rb.velocity.normalized * speedBoost;
