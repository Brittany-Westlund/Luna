...
public class SproutAndLightManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Safety fallback for dynamically assigned children
    ...
    {
        Vector3 startScale = transform.localScale;
        Vector3 endScale = new Vector3(maxScale, maxScale, 1);
        Vector3 startPosition = transform.position;
        float targetHeight = Mathf.Min(transform.position.y + yPositionIncrement, maxHeight);
        Vector3 endPosition = new Vector3(transform.position.x, targetHeight, transform.position.z);
    ...
    {
        if (LightBar != null && ScoreManager.Instance != null)
        {
        ...
        {
            Debug.LogWarning("Missing reference: LightBar or ScoreManager.Instance is null.");
        }
    }
    ...
    /// <summary>
    /// Call this immediately after instantiating the flower prefab to assign necessary references.
    /// </summary>
    public void Setup(MMProgressBar lightBar, Text pointsText, Transform playerTransform)
    {
        LightBar = lightBar;
        PointsText = pointsText;
        PlayerTransform = playerTransform;

        // Dynamically assign child renderer
        Transform litChild = transform.Find("LitFlowerB");
        if (litChild != null)
        {
            LitFlowerRenderer = litChild.GetComponent<SpriteRenderer>();
            if (LitFlowerRenderer != null) LitFlowerRenderer.enabled = false;
        }
        else
        {
            Debug.LogWarning($"LitFlowerB not found under {gameObject.name}");
        }
