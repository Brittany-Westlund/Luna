...
public class FlowerHolder : MonoBehaviour
{
    ...
    public void PickUpFlowerInstance(GameObject flower)
    {
        ...
        if (pivot != null)
        {
            pivot.position = holdPoint.position;
        ...
        ResetWorldScale(flower.transform);
    ...
    private IEnumerator SafePlant(GardenSpot garden)
    {
        ...
        if (pivot != null)
        {
            pivot.position = plantingPoint.position;
            flowerToPlant.transform.SetParent(plantingPoint);
            flowerToPlant.transform.localPosition = Vector3.zero;
        }
        else
        {
            flowerToPlant.transform.position = plantingPoint.position;
            flowerToPlant.transform.SetParent(plantingPoint);
            flowerToPlant.transform.localPosition = Vector3.zero;
        }

        ResetWorldScale(flowerToPlant.transform);
        flowerToPlant.transform.localRotation = Quaternion.identity;
    ...
    private void ResetWorldScale(Transform obj)
    {
        Vector3 lossy = obj.lossyScale;
        if (lossy == Vector3.zero) lossy = Vector3.one;

        Vector3 inverseLossy = new Vector3(
            lossy.x != 0 ? 1f / lossy.x : 1f,
            lossy.y != 0 ? 1f / lossy.y : 1f,
            lossy.z != 0 ? 1f / lossy.z : 1f
        );

        obj.localScale = Vector3.Scale(obj.localScale, inverseLossy);
    }
}
