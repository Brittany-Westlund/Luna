...
public class LuaFunctionsUtility : MonoBehaviour
{
    void Start()
    {
        ...
        Lua.RegisterFunction("MoveRelative", null, typeof(LuaFunctionsUtility).GetMethod("MoveRelative"));
    ...
    public static void MoveRelative(string objectName, double offsetX, double offsetY, double offsetZ)
    {
        GameObject obj = GameObject.Find(objectName);
        if (obj != null)
        {
            Vector3 current = obj.transform.position;
            Vector3 target = current + new Vector3((float)offsetX, (float)offsetY, (float)offsetZ);
            obj.transform.position = target;
            Debug.Log($"Moved '{objectName}' relative by ({offsetX}, {offsetY}, {offsetZ})");
        }
        else
        {
            Debug.LogError($"MoveRelative Error: GameObject '{objectName}' not found.");
        }
    }
