...
public class FairyflyMovement : MonoBehaviour
{
    ...
    private bool resumeLocked = false; // prevents double resume

public System.Action<int> OnDestinationReached;
public System.Action<SproutAndLightManager> OnFlowerLit;
    ...
    // ✅ NEW — external flag to skip waiting once
    [HideInInspector] public bool skipWaitOnce = false;
    ...
    void Start()
    {
        ...
        foreach (var d in destinations)
            if (d != null) worldDestinations.Add(d.position);
    ...
    private IEnumerator MoveSequence()
    {
        ...
        while (currentIndex < worldDestinations.Count)
        {
            ...
            // ✅ Skip waiting once (used after lighting a flower)
            if (skipWaitOnce)
            {
                skipWaitOnce = false; // reset for next cycle
                currentIndex++;
                continue;
            }

            // Normal wait for Luna
            waitingForLuna = true;
   ...
   public IEnumerator MoveSequencePublic()
{
    // 🔒 Prevent double resume if already running
    if (resumeLocked || isMoving)
        yield break;
