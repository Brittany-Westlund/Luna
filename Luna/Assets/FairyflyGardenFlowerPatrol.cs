...
public class FairyflyGardenFlowerPatrol : MonoBehaviour
{
    [Header("Assigned Gardens (only these will be checked)")]
    ...
    private void Start()
    {
        ...
        monitorRoutine = StartCoroutine(MonitorAssignedGardens());
    }
    ...
    private IEnumerator MonitorAssignedGardens()
    {
        ...
        while (true)
        {
            if (!busy)
            {
                SproutAndLightManager targetFlower = GetNextEligibleFlowerFromGardens();
    ...
    private SproutAndLightManager GetNextEligibleFlowerFromGardens()
    {
        for (int i = 0; i < assignedGardens.Count; i++)
        {
            ...
            SproutAndLightManager flower = FindEligibleFlowerInGarden(garden);
            if (flower != null)
            {
                return flower;
            }
    ...
    private SproutAndLightManager FindEligibleFlowerInGarden(Transform garden)
    {
        if (garden == null)
            return null;

        SproutAndLightManager[] flowers = garden.GetComponentsInChildren<SproutAndLightManager>(true);
    ...
    private IEnumerator HandleFlower(SproutAndLightManager flower)
    {
        ...
        if (debugLogs)
            Debug.Log($"{name}: Flying to grown flower {flower.name}");
    ...
    private void ForceIlluminateFlower(SproutAndLightManager flower)
    {
        ...
        // Let the flower finalize its own state/bookkeeping.
