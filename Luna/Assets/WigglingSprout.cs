...
public class FadingSprout : MonoBehaviour
{
    ...
    public float minAlpha = 0.2f; // Minimum alpha value for the sprout
    ...
    void Update()
    {
        ...
        if (wateringScript != null && Time.time >= wateringScript.lastSporeTime + wateringScript.sporeCooldown)
        {
            // Calculate new alpha using a cosine wave for smooth fade in and fade out
            float newAlpha = Mathf.Clamp((Mathf.Cos(Time.time * fadeSpeed) + 1f) * 0.5f, minAlpha, 1f);
