...
public class ButterflyEndSequence : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (restCountdownStarted)
        {
            ...
            if (!dialogueTriggered && timer <= 0f && Vector3.Distance(butterfly.transform.position, luna.position) <= dialogueTriggerDistance)
            {
                TriggerDialogue();
            }
    ...
    void StartSequence()
{
    ...
    {
        dialogueTrigger.enabled = true; // enable it now, but it won’t play yet
    }
    ...
    void TriggerDialogue()
    {
        if (dialogueTrigger != null)
        {
            // Safely handle butterfly speech bubble
            if (butterflySpeechBubble != null)
            {
                var renderer = butterflySpeechBubble.GetComponent<SpriteRenderer>();
                if (renderer != null && !renderer.enabled)
                {
                    renderer.enabled = true;
                    StartCoroutine(HideSpeechBubbleAfterDelay(3f));
                }
            }
