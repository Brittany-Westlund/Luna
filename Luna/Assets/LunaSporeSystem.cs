...
public class LunaSporeSystem : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Prefer same-GO connector; if missing you can still add one later.
        _statusBarConnector = GetComponent<LunaStatusBarConnector>();
        if (_statusBarConnector == null)
        {
            // Optional: reach for any connector in scene if not on this GO.
            _statusBarConnector = FindObjectOfType<LunaStatusBarConnector>();
            if (_statusBarConnector == null)
            {
    ...
    public void UpdateHealthBar()
    {
        ...
        {
            // Let the connector own the UI plumbing (prefab-safe)
            // If you added the overload that accepts values, call that instead:
            // _statusBarConnector.UpdateHealthBar(_healthComponent.CurrentHealth, _healthComponent.MaximumHealth);
            var method = _statusBarConnector.GetType().GetMethod("UpdateHealthBar", System.Type.EmptyTypes);
            if (method != null)
            {
            ...
            {
                // Fallback if you chose the overload with parameters
                _statusBarConnector.SendMessage("UpdateHealthBar",
                    new float[] { _healthComponent.CurrentHealth, _healthComponent.MaximumHealth }, SendMessageOptions.DontRequireReceiver);
            }
        }
    }
