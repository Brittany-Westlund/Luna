...
public class PresentSpore : MonoBehaviour
{
    ...
    // Property to check if a spore is attached
    ...
    // Method to reset the spore state
    public void ResetSporeState()
    {
        if (activeSpore != null)
        {
            Destroy(activeSpore); // Destroy the existing spore
            activeSpore = null;
        }
        IsSliding = false;
        Debug.Log("Spore state has been reset.");
    }
}
